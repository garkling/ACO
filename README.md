...
```
   graph (list[list[int]]): Матриця суміжності графа.
   visibility (list[list[float]]): Матриця видимості (обернені відстані).  
   pheromone (list[list[float]]): Матриця феромонів.  
   params (object): Об’єкт із параметрами алгоритму.  
   node_num (int): Кількість вузлів у графі.  
   path (list[int]): Маршрут, який будується для кожної мурашки.  
```
Повертає:

    path (list[int]): Оновлений маршрут для кожної мурашки.
...
Параметри:
    ...
    pheromone (list[list[float]]): Матриця феромонів.
    path (list[int]): Шляхи, побудовані мурашками.
    graph (list[list[int]]): Матриця суміжності графа.
    params (ACOParameters): Об’єкт із параметрами алгоритму.
    node_num (int): Кількість вузлів у графі.
...
Параметри:
    ...
    path (list[int]): Маршрути мурашок.
    tour_total_distance (list[int]): Загальна довжина маршрутів.

Повертає:

    best_path (list[int]): Найкращий маршрут.
...
Параметри:
    ...
    params (ACOParameters): Об'єкт, який містить параметри алгоритму, включаючи кількість мурашок, кількість ітерацій, швидкість випаровування феромонів та інші налаштування.
